...
// INC-26 P2 (M-19): the posix half of the oracle, generated by
// gen-posix-cases.mjs (Node's path.posix, seeded fuzz, LCG seed
// 20260910) — every line is "p:"-prefixed so the driver above (which
// gained the ONE `p:` arm this pass) routes it to the scr_path_* (posix)
// family instead of the win32 one. Same binary, same file-format,
// same assertion shape as the win32 test above.
test("path.posix functions match Node on committed oracle cases", async () => {
  const { stderr } = await execFileAsync(bin, [join(testDir, "path-cases-posix.txt")]);
  expect(stderr.trim()).toMatch(/^(\d+)\/\1 cases passed$/);
});
