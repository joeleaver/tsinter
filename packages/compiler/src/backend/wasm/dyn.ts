 ...
 *
 * FUTURE COMPOSITE PAYLOADS RIDE `ref` AS TYPED STRUCTS, and FLAGS RIDE
 * PAYLOADS — never extra kind ids. C's ScrDyn carries `null_proto` and
 * `buffer` as bits beside the kind; here OBJ's payload struct will carry
 * the null-prototype flag and BYTES' the Buffer flag, because a new kind
 * id would break the cross-lane numbering above and would make every
 * kind compare in the tier wrong by one arm.
 *
 ...
 * guesses. FUNC boxes arrive with dynCall (stage 4) and composite
 * payloads with the walkers (stage 2); until then their arms say so. */
