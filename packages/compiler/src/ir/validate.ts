...
function validateFunction(
  ...
  function checkExpr(e: IrExpr): void {
    switch (e.kind) {
      ...
      case "arityWiden": {
        // Board #85: identity-preserving pure arity-drop. The predicate is
        // the single source of truth (nodes.ts) — every producer must have
        // already proven it before building this node, so re-checking it
        // here catches a producer bug as a validator error, never a silent
        // miscompile.
        checkExpr(e.value);
        if (!pureArityDrop(e.value.type, e.type)) {
          err("arityWiden operand/result are not a pure arity-drop pair", e.loc);
        }
        break;
      }
      case "upcast":
